...
class DataTextParser final : public DataPipeline {
 public:
  DataTextParser() {}
  ~DataTextParser() final { VLOG(1) << "[DataTextParser] destructor"; }

  std::shared_ptr<StreamMeta> output_stream_meta() const final { return output_stream_meta_; }
...
class DataTextParser final : public DataPipeline {
 public:
  DataTextParser(std::shared_ptr<DataPipeline> input,
                 ...
                 std::string separator, std::vector<DenseColumn> dense_cols,
                 std::vector<SparseColumn> sparse_cols)
      : input_(std::move(input)), separator_(std::move(separator)) {
    // 解析样本字段
    absl::StrSplit(sample_format, '|', &sample_fields_);
    meta_ = std::make_shared<BatchStreamMeta>(std::move(dense_cols), std::move(sparse_cols),
                                              sample_fields_, separator_);
  }
...
#include "data_text_parser.h"
  ...
  static int find_dense_index(const std::shared_ptr<BatchStreamMeta> &meta,
  ...
  static int find_sparse_index(const std::shared_ptr<BatchStreamMeta> &meta,
                               const std::string &name) {
  ...
  absl::StatusOr<std::shared_ptr<Stream>> DataTextParser::next() {
    ...
    while (true) {
      ...
      if (!inflate_stream_) {
        ...
        DATAFLOW_THROW_IF(
            upstream_stream->stream_meta()->stream_type_index() != typeid(InflateStream),
            absl::StrFormat(
                "Upstream must produce InflateStream, got: %s",
                demangle_str_name(upstream_stream->stream_meta()->stream_type_index().name())));
      ...
      batch->mutable_sparse_data().assign(meta_->sparse_columns.size(),
